...
- **Inside exploration-worker / orchestrator spot-verify**: `$mi-lsp` semantic backend under `src/`.
...
- **Closure → push**: `$traceability-check` → `$traceability-audit` (rerun after drift) → Linear sync via `LINEAR_API_KEY` if touched → `$ae-pre-push` before any `git push` to `main`.
...
**mi-lsp** (reference-not-duplicate doctrine — `$mi-lsp` owns command tables, alias validation, intent routing, and sanitized telemetry):
...
10. Before any `git push` to `main`: `$ae-pre-push` after traceability/audit + Linear sync.
