...
func (c *AnalysisCache) Put(k CacheKey, value []byte) error {
    if len(value) > c.max { return NewError("GPH_MILX_OUTPUT_INVALID", "cache", false, "", "cache value exceeds configured bound") }
    c.mu.Lock(); defer c.mu.Unlock()
    c.values[k] = append([]byte(nil), value...)
    return nil
}
func (c *AnalysisCache) Get(k CacheKey) ([]byte, bool) {
    c.mu.RLock(); value, ok := c.values[k]; c.mu.RUnlock()
