...
func extractPython(repo model.WorkspaceRepo, relPath, hash string, content []byte) []model.SymbolRecord {
	...
	walkPythonNode(bt, root, "", &items, repo, relPath, hash)
	return items
}

// walkPythonNode recursively walks the Python AST and extracts symbols.
func walkPythonNode(bt *gotreesitter.BoundTree, node *gotreesitter.Node, parentClass string, items *[]model.SymbolRecord, repo model.WorkspaceRepo, relPath, hash string) {
	if node == nil {
		return
	}
	...
	case "class_definition":
		handleClassDefinition(bt, node, parentClass, items, repo, relPath, hash)
		return // Don't recurse further; handleClassDefinition handles recursion
	case "function_definition":
		handleFunctionDefinition(bt, node, parentClass, items, repo, relPath, hash)
		return // Don't recurse further; handleFunctionDefinition handles recursion
	case "decorated_definition":
		handleDecoratedDefinition(bt, node, parentClass, items, repo, relPath, hash)
	...
	for i := 0; i < node.ChildCount(); i++ {
		child := node.Child(i)
		if child != nil {
			walkPythonNode(bt, child, parentClass, items, repo, relPath, hash)
		}
	}
}

// handleClassDefinition extracts class definitions and recursively processes their methods.
func handleClassDefinition(bt *gotreesitter.BoundTree, node *gotreesitter.Node, parentClass string, items *[]model.SymbolRecord, repo model.WorkspaceRepo, relPath, hash string) {
	...
	*items = append(*items, record)
	...
	if bodyNode != nil {
		walkPythonNode(bt, bodyNode, className, items, repo, relPath, hash)
	}
}

// handleFunctionDefinition extracts function or method definitions.
func handleFunctionDefinition(bt *gotreesitter.BoundTree, node *gotreesitter.Node, parentClass string, items *[]model.SymbolRecord, repo model.WorkspaceRepo, relPath, hash string) {
...
func handleDecoratedDefinition(bt *gotreesitter.BoundTree, node *gotreesitter.Node, parentClass string, items *[]model.SymbolRecord, repo model.WorkspaceRepo, relPath, hash string) {
	...
	for i := 0; i < node.ChildCount(); i++ {
		...
		if child != nil {
			...
			switch childType {
			case "class_definition":
				handleClassDefinition(bt, child, parentClass, items, repo, relPath, hash)
				return
			case "function_definition":
				handleFunctionDefinition(bt, child, parentClass, items, repo, relPath, hash)
				return
			}
		}
	}
