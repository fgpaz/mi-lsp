...
|---|---|---|---|
| Claude Code | sí, `C:\Users\fgpaz\.local\bin\claude.exe` | no hay subcomando `plugin validate`. No se instaló el marketplace ni se arrancó sesión. Pruebas Node de hooks: 14 pass | no |
| Grok | sí, `C:\Users\fgpaz\.grok\bin\grok.exe` | `grok plugin validate integrations/grok`, exit 0: manifiesto válido, nombre `mi-lsp`, versión 0.1.0, componente MCP. No se instaló el plugin | no |
