...
- Indexacion: `workspace.add/init` indexan de forma **sincronica y acotada por default** para preservar el contrato init-then-query (el caller puede consultar inmediatamente). El indexado async-first (D6) queda como modo de primera clase opt-in via `--background`/`background:true` para workspaces muy grandes, devolviendo `job_id` al instante. El default sincronico esta acotado por `IndexTimeout` (`MI_LSP_INDEX_TIMEOUT`, 5min) para no colgar indefinidamente (AUD-01); un timeout degrada a warning en vez de bloquear. `--wait` se conserva como no-op de compatibilidad (sync ya es el default).
...
- `workspace.add` y `workspace.init` indexan **sincronicamente por default** (acotado por `IndexTimeout`); `--background` indexa en background y devuelve `job_id` para workspaces muy grandes; `--no-index` omite indexacion. `--wait` es no-op de compatibilidad.
