...
1. El watcher observa cambios de archivos en el workspace (.cs, .ts, .tsx, .py).
2. Emite evento de cambio (add/mod/del).
3. Aplica debounce de 500ms para evitar procesamiento frenético.
4. Extrae simbolos del archivo modificado.
5. Actualiza `index.db` con nuevos simbolos / deletea si se removio archivo.
6. Operacion no-blocking en background.
