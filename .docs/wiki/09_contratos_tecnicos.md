...
- `workspace doctor`: diagnostico no mutante de aliases con root compartido, worktrees con mismo `git common dir`, paths stale, shadowing de binario, metadata del binario activo y binarios visibles (`revision`, `goos`, `goarch`, `modified`), drift de revision contra el binario activo, `health`, `next_actions` y sugerencias de comandos.
