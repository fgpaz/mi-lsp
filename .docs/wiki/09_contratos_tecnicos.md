...
- `nav search <pattern>`: si `--regex` lleva un patron invalido, el runtime puede reintentar como literal y devolver warning explicito en vez de error duro; si el patron literal parece identificador de codigo, debe emitir guidance agent-first hacia `nav find --exact`/`nav related` y rankear fuentes antes que docs/tests/backups/generados; si `rg` falla por permisos/arranque, debe caer a Go search con warning `backend_runtime/<code>`; si agota el presupuesto textual interactivo (5s por workspace) con parciales seguros, conserva `ok=true`, emite warning `search_timeout`, `next_hint` y `coach.trigger=search_timeout`
