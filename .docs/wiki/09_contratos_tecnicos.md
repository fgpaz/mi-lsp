...
- el export raw de `access_events` preserva metadata operativa minima del request (`route`, `format`, `token_budget`, `max_items`, `max_chars`, `compress`) para diferenciar uso directo, daemonizado y truncacion por presupuesto; en operaciones daemonizadas normales debe existir una sola fila canonica por request
