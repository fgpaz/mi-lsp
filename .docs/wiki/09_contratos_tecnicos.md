...
- El safe-degrade planner es parte del contrato de precision: debe elegir la superficie mas barata que preserve autoridad (`route|wiki search|pack preview` antes de lecturas o semantica profunda), degradar con warning/hint tipado, y registrar solo metadatos sanitizados (`planner_path`, `planner_outcome`, `safe_degrade_reason`, `guardrail_trigger`) dentro de `decision_json`.
...
- Graph envelopes agregan `operation`, `generation_id`, `graph_schema_version`, `omissions` y `determinism_digest`; `omissions[]` puede exponer contexto documental opcional (`source_document`, `source_block`, `target_kind`, `target_value`), nullable en persistencia y acotado a 4096 bytes; la serialización sanea la ruta de `source_document` (omitiendo valores absolutos o con traversal) y limpia/limita el texto de los demás campos (omitiendo controles). Cursors se ligan a generation/query/ordering.
...
- Error codes `GPH_*` son tipados y sanitizados. Truncation, member unavailable y omission no se presentan como completitud ni se esconden en texto libre; los diagnósticos documentales conservan `missing_doc_target` o `ambiguous_doc_target` como `reason_code` tipado.
...
- `nav trace <DOC-ID>` debe clasificar explícitamente `RS|RF|TP`: `RS-*` devuelve `doc_id`, `layer=RS`, `stage=outcome` sin poblar el campo legacy `rf`; `RF-*` conserva evidencia spec-to-code y `TP-*` conserva evidencia de cobertura documental. En el grafo documental, una mención `TP-*` usa coincidencia exacta de `DocRecord.doc_id`: si hay un único target se publica nodo/edge; si falta o está duplicado, se conserva `GraphUnresolved` tipado con `reason_code=missing_doc_target|ambiguous_doc_target`, `target_kind=document`, `target_value` solicitado y candidatos bounded, sin elegir un candidato arbitrario.
...
- `nav wiki map`: catálogo compacto de hubs de una wiki de conocimiento (`persona`, `proyectos`, `sistema`, `materia`) sobre `wiki/` y `bibliotecas/`; single-workspace; recorta por `--token-budget`/`MaxItems`; no emite cuerpos completos
...
- `nav ask <question>`: responde usando wiki + evidencia de codigo y fallback generico/textual cuando haga falta; normaliza meta-terminos SDD en preguntas de anclas y puede emitir `coach.trigger=anchor_drift`; `--all-workspaces` habilita fan-out cross-workspace para el mismo contrato explainable
- `nav pack <task>`: construye un reading pack canonico con preview/full y anchors opcionales `--rf`, `--fl`, `--doc`
- `nav search <pattern>`: si `--regex` lleva un patron invalido, el runtime puede reintentar como literal y devolver warning explicito en vez de error duro; si el patron literal parece identificador de codigo, debe emitir guidance agent-first hacia `nav find --exact`/`nav related`, rankear fuentes antes que docs/tests/backups/generados y acotar el sobre-muestreo en AXI preview; si `rg` falla por permisos/arranque, debe caer a Go search con warning `backend_runtime/<code>`; si usa `rg --hidden`, debe conservar visibilidad de `.docs` y aplicar globs de exclusion para `.git`, caches, dependencias y build outputs por defecto; si agota el presupuesto textual interactivo (5s por workspace) con parciales seguros, conserva `ok=true`, emite warning `search_timeout`, `next_hint` y `coach.trigger=search_timeout`; si `--include-content` encuentra un path indexado que ya no existe, debe conservar el match y emitir stale-index warning accionable hacia reindex.
- `workspace status [--full] [--no-auto-sync]`: muestra `workspace_root`, `workspace_source`, estado de gobernanza/index y detalles de stale index con timestamps comparados; en `--full`, el digest repo-local de memoria (`recent_canonical_changes`, `handoff`, `best_reentry`, `stale`); si la memoria esta stale, `auto_sync` esta habilitado y la gobernanza no esta bloqueada, puede ejecutar refresh docs-only y devolver warning visible; `--no-auto-sync` conserva la consulta read-only y reporta `governance_sync=stale` sin escribir `read-model.toml` ni refrescar memoria; si `docs_index_ready=true` pero `index_ready=false`, debe explicitar que el corpus docs-only quedo util pero el catalogo de codigo sigue ausente
...
- `workspace doctor`: diagnostico no mutante de aliases con root compartido, worktrees con mismo `git common dir`, paths stale, readiness documental/gobernanza de aliases vivos, colisiones case-insensitive en el tree Git, shadowing de binario, metadata del binario activo y binarios visibles (`revision`, `goos`, `goarch`, `modified`), drift de revision contra el binario activo, `health`, `next_actions` y sugerencias de comandos. Cuando detecta colisiones de casing debe exponer `git_case_collisions` y una accion `fix_git_case_collisions` sin modificar registry, worktrees ni el repo afectado.
- cuando `--workspace` se omite, `nav ask`, `nav pack`, `nav governance`, `workspace status` y las queries directas equivalentes resuelven primero contra `caller_cwd`; si no hay match, pueden usar `last_workspace` con warning visible. Si `--workspace <alias>` esta presente y el `caller_cwd` cae dentro de otro workspace registrado, callers humanos preservan compatibilidad con warning accionable; callers agente/harness rechazan el mismatch con `error.code=workspace_cross_workspace_refused` y `hint_code=workspace_cross_workspace_refused`, salvo override global `--allow-cross-workspace`. El override debe conservar warning visible para auditoria.
- `nav governance`: diagnostica perfil efectivo, sync, stale index con timestamps comparados y pasos de reparacion de gobernanza
