...
Only these external fallback reasons are allowed: `unsupported_operation`, `unavailable_binary`, `invalid_workspace`, and `explicit_incomplete`. Each one needs a separate sanitized `detail`. If none is visible, stay with `mi-lsp` and report the limitation. `mi-lsp mcp` and `integrations/` host doors are optional; the CLI remains the authority. Use 180/300-second soft/hard watchdogs, at most two same-context recoveries with a smaller packet and no unchanged retry, at most six practical lanes with exclusive `allowed_paths`, fail-closed joins, fresh verification, and redacted evidence without prompts, transcripts, secrets, PII, PHI, argv, or raw patterns. Do not invent model/provider metadata.
